...
export async function GET(req: NextRequest) {
    ...
    const { data: user } = await supabaseAdmin.from('profiles').select('id').eq('email', 'simonsilverboombox@gmail.com').maybeSingle()
    ...
    return NextResponse.json({ success: true, user_id: user.id, count: records.length })
}
